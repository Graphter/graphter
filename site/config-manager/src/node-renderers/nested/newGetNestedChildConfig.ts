...
export const newGetNestedChildConfig: NewGetChildConfigFn = (config, path, childSegment, getPropValue) => {
  ...
  return nestedRendererReg.newGetChildConfig ?
    nestedRendererReg.newGetChildConfig(nestedConfig, path, childSegment, getPropValue) :
    null
}