...
describe('getChildData()', () => {
  it('should return descendent data correctly formed', () => {
    ...
    const result = getObjectChildData(config, ['page'], jest.fn())
    expect(result).toEqual({
      author: { name: 'Bob', location: 'London' }
    })
  })
  it.each([ null, undefined ])('should error if %o parent config is not found', (noConfig) => {
    // @ts-ignore
    expect(() => getObjectChildData(noConfig, ['page'], jest.fn()))
  ...
  it.each([[], null])
    ...
    // @ts-ignore
      expect(() => getObjectChildData(config, ['page'], jest.fn()))
      .toThrowErrorMatchingSnapshot()
  })
  it('should skip descendent data resolution when child renderer does not implement a getChild() function', () => {
    ...
    const result = getObjectChildData(config, ['page'], getNodeValueMock)
    expect(result).toEqual({
